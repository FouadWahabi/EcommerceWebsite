...
class register_model extends model {
    ...
    function addUser($data) {
        if($this->isUserExists($data['email-reg']) === 'false') {
            ...
            $sth->setFetchMode(PDO::FETCH_ASSOC);
            $sth->execute(array(':first_name' => $data['first_name'], ':last_name' => $data['last_name'], ':email_reg' => $data['email-reg'], ':password_reg' => hash('sha256', $data['password-reg']), ':phone' => $data['phone'], ':city' => $data['city'], ':state' => $data['state'], ':zip' => $data['zip'], ':adress' => $data['adress']));
            Redirect(URL);
        } else {
            // handle error
        }
    ...
    function signIn($data) {
        $sth = $this->db->prepare('SELECT * FROM e_users WHERE user_email = :email AND UPPER(user_pswd) = UPPER(:pswd)');
        $sth->setFetchMode(PDO::FETCH_ASSOC);
        $sth->execute(array(':email' => $data['user_email'], ':pswd' => hash('sha256', $data['user_pswd'])));
        $user = $sth->fetchAll();
        if(count($user) > 0)
